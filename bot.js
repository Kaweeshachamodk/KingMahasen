...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        ...
        console.log(chalk.bgGreen('𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷 v 1.0 _____ 🤞😎🔰' + afwhasena));
        await new Promise(r => setTimeout(r, 500));
        let EVA_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '*𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷 චැට්බෝට් ලෙස ක්‍රියා කරයි!*\n\n_මෙම  mode හි වන්නේ බොට් සම්පූර්ණයෙන්ම ක්‍රියාකාරී AI චැට් මෙවලමක් බවට පත් කිරීමයි.._\n_සාමාන්‍ය ප්‍රකාරයට ආපසු යාමට_ *.fulleva off* _ඔබට විධානය භාවිතා කළ හැකිය._\n\n*𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷 භාවිතා කිරීමට ස්තූතියි 💌*\n    *- Eva*' : '*𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷 Working as a Chatbot! *\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fulleva off* _command to return to normal mode._\n\n*Thanks For Using 𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷 💌*\n    *- Eva*'
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷 පොදු ලෙස ක්‍රියා කරයි!*\n\n_කරුණාකර මෙහි ප්ලගීන උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකය වේ._\n_ඔබට ඕනෑම කතාබහකදී විධාන උත්සාහ කළ හැකිය :)_\n\n*ඔබේ බොට් ප්‍රසිද්ධියේ ක්‍රියාත්මක වේ. එය වෙනස් කිරීමට* _.setvar WORK_TYPE:private_ *විධානය භාවිතා කරන්න.*\n\n*𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷 භාවිතා කිරීම ගැන ස්තූතියි 💌*', MessageType.text);
                }
                await git.fetch();
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷 පුද්ගලික ලෙස ක්‍රියා කරයි!*\n\n_කරුණාකර මෙහි ප්ලගීන උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකය වේ._\n_ඔබට ඕනෑම කතාබහකදී විධාන උත්සාහ කළ හැකිය :)_\n\n*ඔබේ බොට් ප්‍රසිද්ධියේ ක්‍රියාත්මක වේ. එය වෙනස් කිරීමට* _.setvar WORK_TYPE:public_ *විධානය භාවිතා කරන්න.*\n\n*𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷 භාවිතා කිරීම ගැන ස්තූතියිr 💌*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '*-- HATA RAPORU [𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷] --*' + 
                                    '\n*𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷 bir hata gerçekleşti!*'+
                                    '\n_Bu hata logunda numaranız veya karşı bir tarafın numarası olabilir. Lütfen buna dikkat edin!_' +
                                    '\n_Yardım için Telegram grubumuza yazabilirsiniz._' +
                                    '\n_Bu mesaj sizin numaranıza (kaydedilen mesajlar) gitmiş olmalıdır._' +
                                    '\n_Hatayı https://chat.whatsapp.com/EztcSyEFSXILEDK2Yghdxr bu gruba iletebilirsiniz._\n\n' +
                                    '*Gerçekleşen Hata:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false});

                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ HATA ÇÖZÜMLEME [𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*🙇🏻 Maalesef Bu Hatayı Okuyamadım! 🙇🏻*' +
                                        '\n_Daha fazla yardım için grubumuza yazabilirsiniz.https://chat.whatsapp.com/EztcSyEFSXILEDK2Yghdxr_'
                                        , MessageType.text
                                    );
                                }
                            }
                            else {
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷] --*' + 
                                    '\n*𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷 an error has occurred!*'+
                                    '\n_This error log may include your number or the number of an opponent. Please be careful with it!_' +
                                    '\n_You can write to our Telegram group for help._' +
                                    '\n_Aslo you can join our support group:_ https://chat.whatsapp.com/EztcSyEFSXILEDK2Yghdxr' +
                                    '\n_This message should have gone to your number (saved messages)._\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [𝓚𝓲𝓷𝓰𝓜𝓪𝓱𝓪𝓼𝓮𝓷] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻*' +
                                        '\n_You can write to our support group for more help.https://chat.whatsapp.com/EztcSyEFSXILEDK2Yghdxr_'
                                        , MessageType.text
                                    );
                                }    
                            }                      
